define({
  "viewer": {
    ...
    "builderHTML": {
      ...
      "settingsLayoutSidePanel": "サイドパネル",
      ...
      "dataExplainPortal": "この機能は Portal for ArcGIS 10.4 を必要とします。フィーチャレイヤーの作成に失敗する場合は、ArcGIS 管理者に連絡してください。",
      "dataNameLbl": "レイヤー名",
      ...
      "coverPreview": "カバーを表示するには、ストーリーを保存した後、[共有] ボタンを使用してプレビューします。"
    },
    "addPopupJS": {
    ...
    "viewPicasa": {
      "title": "Picasa のインポート",
      "header": "電子メール アドレスまたは Picasa ID を入力します。",
      "userInputLbl": "電子メールまたは Picasa ID",
    ...
    "viewGeoTag": {
      ...
      "headerExplain": "写真に有効な位置情報が含まれている場合、それらの写真は自動的にマップ上に配置され、2 番目のタブにリストされます。<br /><br />デフォルトでは、写真をインポートするときに、Picasa と Flickr は位置情報の EXIF メタデータを保存しません。 Picasa/Flickr のプライバシー設定で、写真の位置情報のインポートと外部アプリケーションによる使用が有効化されていることを確認してください。 EXIF の位置情報を使用するために、既存の写真を Flickr/Picasa に再インポートすることが必要になる場合があります。",
      "leftPanelTab1": "検索する",
      "leftPanelTab2": "検索済み",
    ...
    "helpPopup": {
      ...
      "tab1": {
        ...
        "mtFAQ": "<a href='https://storymaps.arcgis.com/en/faq/#category4' target='_blank'>マップ ツアーの FAQ</a>",
      ...
      "tab2": {
        ...
        "div11": "自身で写真をホストする場合は、写真のサムネイルを手動で作成する必要があります。サムネイルにフル解像度の写真を使用すると、パフォーマンスが低下します。URL 経由で直接 Web 上からアクセスする場合は、この <a href='https://links.esri.com/storymaps/faq_maptour_image_size' target='_blank'>FAQ</a> で画像のサイズ設定に関するヒントをご参照ください。",
      ...
      "tab4": {
        ...
        "div1": "イメージ",
        "div2": "サイズ、形状、方向の異なる画像を 1 つのマップ ツアーで使用することはできますが、縦向きではなく横向きの画像の使用をお勧めします。縦横比が 4:3 の画像が最適です。3 パネルおよび統合レイアウトでは、キャプションが画像と重なるため、縦向き画像を使用すると、多くの画像がキャプションに隠されてしまう可能性があります。3 パネルおよび統合レイアウトでは、可能な限り、すべての画像に同じサイズと形状を使用することをお勧めします。こうすることで、ユーザーがツアー中に、サイズの異なる画像に混乱させられることがありません。",
        ...
        "div4b": "ツアーに説得力のあるサブタイトルを書き込んでください。ユーザーは、サブタイトルを見てツアーに注目し、その内容を理解します。また、ユーザーにツアーの地方や国を伝えることもできます。たとえば、ユーザーはツアーの市や町がどこにあるかわからないと考えてください。ただし、サブタイトルは長すぎないようにしてください。適切なサブタイトルが考え付かない場合は、空欄にしておいてください。サイドパネル レイアウトでは、カバー ページに表示されるサブタイトルは、ツアーの最初のポイントのキャプションです ([組織] ダイアログのオプションを使用して、最初のポイントのコンテンツをカバー ページとして使用している場合)。",
        "div5": "レイヤのサポート",
        "div6": "さらにサポート レイヤーを追加して、マップ ツアーにコンテキストを提供できます。これらのレイヤーには、マップ ツアー ポイントに加えてマップに表示したい、分析範囲、ツアー ポイントを接続するウォーキング ルート、ドライブ ルートなどのジオグラフィック フィーチャを含めることができます。マップ ツアー テンプレートは、Web マップで指定したシンボルを使用して、これらの追加のサポート レイヤーを表示します。ポップアップを使用することはできません。サポート レイヤーをツアーに追加するには、作業内容を保存し、マップ ツアー ビルダーを閉じます。次に、ツアーで使用される Web マップを開きます。Web マップは、ArcGIS の [マイ コンテンツ] か、ストーリー マップ Web サイトの [マイ ストーリー] セクションにあります。",
        "div7": "ツアーを短く魅力的に",
        ...
        "div12": "マップ ツアーが反応しやすい小画面のタッチベースのレイアウトに切り替わってしまう原因となる狭い iFrame 幅は使用しないようにします。できるだけ使いやすくするために、マップ ツアーを埋め込むときは、ユーザーがツアーを全画面で起動できるように、埋め込まれたツアーの横にリンクを設定することを常にお勧めします。"
      },
      "tab5": {
        "title": "公開",

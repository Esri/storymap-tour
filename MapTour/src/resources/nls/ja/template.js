  ...
  "viewer": {
    ...
    "helpPopup": {
      ...
      "tab2": {
        ...
        "div3": "<ul><li>Flickr の画像や独自の Web サーバーに保存された画像など、</b>すでにオンラインに存在する写真</b>を使用できます。 それらの画像は、URL を使用してマップ ツアーで参照されます。</li><li><b>写真をコンピューターから直接マップ ツアーにアップロードする</b>こともできます。 このアップロードのオプションを使用するには、ArcGIS サブスクリプションを所有し、公開者権限または管理者権限を持っている必要があります。これは、ホスト フィーチャ サービスが自動的に作成され、写真が添付ファイルとしてそのフィーチャ サービスに保存されるためです。</li></ul>",
        ...
        "div11": "(Flickr の画像にアクセスしたり、ツアーに直接アップロードするのではなく) 独自の Web サーバーで写真をホストすることを選択した場合、写真のサムネイルを手動で作成する必要があります。 サムネイルにフル解像度の写真を使用すると、パフォーマンスが低下します。 URL 経由で直接 Web 上でアクセスする場合の画像のサイズ設定については、こちらの「<a href='https://links.esri.com/storymaps/faq_maptour_image_size' target='_blank'>FAQ</a>」をご参照ください。",
        "div12": "既存の場所データの使用",
        "div13": "スクリーンショットや GIS ポイント レイヤー内の場所のリストなど、既存の場所データがある場合、そのデータを CSV 形式で使用してマップ ツアーを作成できます。 これには、CSV ファイル内に値として保存されている URL を介して、写真に直接アクセスする必要があります。 最初にマップ ツアー ビルダーを起動するときに、[高度な設定] ダイアログから CSV テンプレートをダウンロードできます。 CSV ファイルはそのダイアログにアップロードできます。",
        ...
        "div174": "詳細については、こちらの「<a href='https://links.esri.com/storymaps/faq_map_tour_gis_data' target='_blank'>FAQ</a>」をご参照ください。",
        ...
        "div21": "Flickr の画像にアクセスすると、マップ ツアーは *.jpg ファイルとしてこれらの画像に自動でアクセスします。 マップ ツアー ビルダーに画像をアップロードする場合、画像の形式は *.jpg または *.jpeg である必要があります。 URL 経由で直接 Web 上で画像を参照する場合、サポートされる写真の形式は、*.jpg、*.jpeg、*.png、*.gif、および *.bmp です。 YouTube ビデオは直接サポートされます。",

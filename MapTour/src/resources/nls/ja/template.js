...
({
		viewer: {
			...
			builderHTML: {
				...
				dataPicError4: "このアクションでは、サポートされていない写真 URL の末尾に <i>#isPicture</i> を追加します。ほとんどのサーバでは URL の文字列追加がサポートされていますが、このアクションを実行した後は、ポイント間を移動することで、更新した写真 URL が正常に機能することを確認する必要があります。それぞれの写真が読み込まれた場合は、マップ ツアーを保存できます。<b>写真のリンクが切れている場合は、マップ ツアーを保存しないでください。</b>代わりに、ビルダを再び読み込んで、2 番目のアクションを実行してください。",
			...
			addPopupJS: {
				...
				errorInvalidPicUrl: "このツアー ポイントの有効な写真 URL (http(s):// で始まり、jpg、png、gif、または bmp で終わる) を入力してください。URL の末尾を｢#isPicture｣にすると、このルールを無視できます。",
			...
			viewGeoTag: {
				...
				footerSucceed: "インポートに成功しました。読み込んでいます...",
				loading: "読み込んでいます",
				error: "写真の場所のインポートに失敗しました。場所が無視されました。"
			},
			initPopup: {
			...
			helpPopup: {
				...
				tab1: {
					...
					div2: "このテンプレートを使用して、魅力的な使いやすい Web アプリケーションを作成できます。そのWeb アプリケーションでは、番号の順序でマップ上に小規模な一連の場所が表示され、その順序で参照できます。このテンプレートは、スマートフォンやタブレットなど、任意のデバイス上の任意の Web ブラウザで使用するように設計されています。",
				...
				tab2: {
					...
					div3: "<ul><li>Flickr のような写真共有サイトに保存された画像や、独自の Web サイトに保存された画像などの、<b>すでにオンラインに存在する写真</b>を使用できます。それらの画像は、URL を使用してマップ ツアーで参照されます。</li><li><b>写真をコンピューターから直接マップ ツアーにアップロードする</b>こともできます。このアップロードのオプションを使用するには、ArcGIS Online サブスクリプションを所有し、公開者権限または管理者権限を持っている必要があります。これは、ホスト フィーチャ サービスが自動的に作成され、写真がアタッチメントとしてそのフィーチャ サービスに保存されるためです。</li></ul>",
					...
					div154: "サムネイル",
					div155: "色 (オプション)",
					div16: "[名前] フィールドと [説明] フィールドは必須です。フィーチャ サービスを使用しているときに、一致するフィールドをアプリケーションが検出できない場合、ビルダで使用されるフィールドを構成するまで、ビューアは動作しません。Web マップに追加される CSV とシェープファイル レイヤには、必須フィールドがすべて存在している必要があります。必須フィールドが存在しない場合、ビルダは動作しません。",
				...
				tab4: {
					...
					div1: "写真",
					div2: "縦方向ではなく横方向の写真を使用することをお勧めします。縦方向の画像を使用することはできますが、iPad のような小さい画面上では、大量の写真がキャプションによって不明瞭になることがあります (テキストを縦長の領域に表示すると、横長の領域に表示する場合よりも多くのスペースが使用されます)。マップ ツアーでは、さまざまなサイズ、形状、および方向の画像を使用できますが、正確に同じサイズと形状をすべての画像に対して使用することをお勧めします。そうすることで、ユーザがツアーをたどるときに、異なるサイズの画像に気を取られることがなくなります。",
					div2a: "写真を自分でホストしている場合は、メインの写真には <b>1090x725</b>、サムネイルには <b>140x93</b> の最大解像度を推奨します。",
				...
				tab5: {
					title: "公開",
					div1: "完了したら、必ずマップ ツアーを、ArcGIS Online のアプリケーションの詳細ページを使用して利用者と共有してください。この処理の実行中に、ArcGIS Online は、同じように共有されていない依存するリソース (Web マップ、フィーチャ サービス) を更新するかどうかを、必要に応じて尋ねます。マップ ツアーがパブリックである場合にリソースのうちのいずれかが利用者と共有されていないときは、ユーザは、ArcGIS Online のログイン ページにリダイレクトされます。",
					div2: "マップ ツアー ビルダによって作成された、ホスト フィーチャ サービスを使用している場合、アプリケーションはサービスのセキュリティを自動的に管理します。サービスをパブリックに共有している場合でも、編集権限を持つのはあなただけです。",
					div3: "マップ ツアーをパブリックに共有する前に、ArcGIS.com アカウントでログインしていない状態で、そのマップ ツアーが動作することを確認してください。",
					div4: "iPad を横向きにしてマップ ツアーの表示を確認してみることをお勧めします。これにより、写真上のキャプション表示領域が大きすぎないか確認できます。"
				}
			}
        }
    })

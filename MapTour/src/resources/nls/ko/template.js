define({
  "viewer": {
    ...
    "builderHTML": {
      ...
      "settingsLayoutProfessional": "세 개의 패널",
      ...
      "dataExplainPortal": "이 피처를 사용하려면 Portal for ArcGIS 10.4가 필요합니다. 피처 레이어를 생성할 수 없는 경우 ArcGIS 관리자에게 문의하세요.",
      "dataNameLbl": "레이어 이름",
      ...
      "coverPreview": "커버를 보려면 스토리를 저장한 다음, 공유 버튼을 사용하여 미리 봅니다."
    },
    "addPopupJS": {
    ...
    "viewPicasa": {
      "title": "Picasa 가져오기",
      "header": "이메일 주소나 Picasa ID를 입력합니다.",
      "userInputLbl": "이메일 또는 Picasa ID",
      "signInMsg2": "계정을 찾을 수 없음",
      "signInMsg3": "공개 앨범 없음",
      "howToFind": "Picasa ID를 찾는 방법",
    ...
    "viewGeoTag": {
      ...
      "headerExplain": "사진에 유효한 지오로케이션이 있는 경우 맵에 자동으로 위치가 지정되고 두 번째 탭에 나열됩니다.<br /><br />기본 설정에 따라 피카사(Picasa) 및 플리커(Flickr)는 사진을 가져올 때 지오로케이션 EXIF 메타데이터를 저장하지 않습니다. 피카사/플리커 개인 정보 보호 설정을 확인하여 외부 응용프로그램에서 사진 지오로케이션의 가져오기 및 사용을 활성화합니다. EXIF 지오로케이션을 사용하려면 기존 사진을 피카사/플리커에 다시 가져와야 할 수도 있습니다.",
    ...
    "helpPopup": {
      ...
      "tab1": {
        ...
        "mtFAQ": "<a href='https://storymaps.arcgis.com/en/faq/#category4' target='_blank'>맵 투어 FAQ</a>",
      ...
      "tab2": {
        ...
        "div3": "<ul><li>플리커 등의 사진 공유 사이트에 저장된 이미지나 자신의 웹 사이트에 저장된 이미지와 같이 <b>온라인에 있는 이미지 사진</b>을 사용할 수 있습니다. 이러한 이미지는 해당 URL을 통해 맵 투어에서 참조됩니다.</li><li><b>컴퓨터에서 맵 투어로 직접 사진을 업로드</b>할 수도 있습니다. 이 업로드 옵션을 사용하려면 사진이 첨부 파일로 저장되는 호스팅된 피처 서비스가 자동으로 생성되므로 ArcGIS 구독이 있어야 하며 발행자 또는 관리자 권한이 있어야 합니다.</li></ul>",
        ...
        "div10": "웹 서버에 그림 저장",
        "div11": "사진을 직접 호스팅하도록 선택하려면 사진의 섬네일을 수동으로 생성해야 합니다. 섬네일에 전체 해상도 사진을 사용하면 성능이 저하됩니다. URL을 통해 웹에서 직접 이미지에 접근할 경우 이 <a href='https://links.esri.com/storymaps/faq_maptour_image_size' target='_blank'>FAQ</a>를 참고하여 이미지 크기 조정에 대한 팁을 얻으세요.",
        "div12": "기존 GIS 데이터 사용",
      ...
      "tab4": {
        ...
        "div1": "이미지",
        "div2": "하나의 맵 투어에서 다양한 크기, 도형 및 방향의 이미지를 사용할 수는 있지만 세로 방향보다는 가로 방향의 이미지를 사용하는 것을 권장합니다. 가로 세로 비율이 4:3인 이미지가 가장 적합합니다. 캡션이 이미지와 중첩되는 세 개의 패널 및 통합 레이아웃에서 세로 방향 이미지를 사용하면 많은 이미지가 캡션에 의해 가려질 수 있습니다. 세 개의 패널 및 통합 레이아웃에서는 가능한 경우 크기와 도형이 모두 동일한 이미지를 사용하는 것을 권장합니다. 그러면 사용자가 투어를 진행할 때 크기가 다른 이미지 때문에 혼동되는 일이 발생하지 않습니다.",
        ...
        "div4b": "여유를 갖고 투어에 사용할 멋진 부제목을 작성해 보세요. 부제목을 통해 사람들의 이목을 집중시키고 투어의 내용을 전달할 수 있습니다. 투어가 소재한 국가나 시/도 정보를 대상 그룹에게 알려줄 수도 있습니다. 관련 정보를 제공하지 않으면 사람들은 투어가 진행되는 시/군/구 또는 장소를 알 수 없습니다. 부제목은 너무 길지 않게 작성하세요. 적절한 부제목이 생각나지 않으면 비워 두세요. 측면 패널 레이아웃에서 커버 페이지에 나타나는 부제목은 투어의 첫 번째 지점에 대한 캡션입니다(구성 대화 상자의 옵션을 사용하여 첫 번째 지점의 콘텐츠를 커버 페이지로 사용하는 경우).",
        "div5": "보조 레이어",
        "div6": "보조 레이어를 추가하여 맵 투어에 컨텍스트를 제공할 수 있습니다. 이러한 레이어는 맵 투어 포인트 외에 연구 영역, 투어 포인트에 연결되는 도보 또는 운전 경로 등 맵에 표시할 기타 지리 피처를 포함할 수 있습니다. 맵 투어 템플릿은 웹 맵에서 지정한 심볼을 사용하여 이러한 추가 지원 레이어를 표시하지만, 팝업은 사용할 수 없습니다. 투어에 지원 레이어를 추가하려면 작업을 저장한 후 맵 투어 빌더를 닫습니다. 그런 다음, 투어에 사용된 웹 맵을 엽니다. 이러한 지원 레이어는 ArcGIS 내 콘텐츠나 스토리맵 웹 사이트의 내 스토리 섹션에서 찾을 수 있습니다.",
        "div7": "간결하게 Tour 유지",
        ...
        "div12": "너비가 좁은 iFrame은 맵 투어를 반응형 소형 화면 터치 기반 레이아웃으로 전환되게 하므로 사용하지 마세요. 맵 투어를 최대한 간편하게 사용하려면 맵 투어를 임베드할 때 사용자가 임베드된 투어를 전체 화면에서 시작할 수 있도록 해당 투어 옆에 링크를 제공하는 것을 항상 권장합니다."
      },
      "tab5": {
        "title": "발행",
    ...
    "share": {
      ...
      "manageStory": "스토리 관리",
      "manageStoryA1": "팁: %LINK1%에서 스토리에 오류가 있는지 확인하고 컴포넌트 공유 방법을 변경할 수 있습니다. 내 스토리에서도 스토리가 소셜 네트워크에서 공유될 때의 스토리 모양을 멋있게 꾸밀 수 있습니다. 해당 %LINK2%에서 기타 유용한 내 스토리 기능에 대해 알아볼 수 있습니다.",
      "manageStoryA1V1": "내 스토리",
      "manageStoryA1V2": "블로그 게시물",
      "shareTitle": "Tour 공유",

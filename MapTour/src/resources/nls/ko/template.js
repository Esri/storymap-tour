define({
  "viewer": {
    ...
    "errors": {
      ...
      "noLayerMobile": "맵 투어 빌더는 이 디스플레이 크기에서 지원되지 않습니다. 가능한 경우 빌더에 접근할 수 있도록 브라우저의 크기를 조정하거나 화면이 더 큰 기기에서 스토리를 만드세요.",
      "noLayerMobile2": "맵 투어 빌더를 사용하려면 기기를 가로 방향으로 회전하세요.",
      "noLayerView": "맵 투어 웹 응용프로그램을 시작합니다.<br />스토리가 아직 구성되지 않았습니다.",
      ...
      "notAuthorizedBuilder": "맵 투어 빌더를 사용할 수 있는 권한이 없습니다.",
    ...
    "builderHTML": {
      ...
      "dataSourceWarning": "맵 투어 데이터 레이어가 변경되었습니다. 피처 ID가 동일하지 않은 경우 <b>구성</b>을 통해 순서 및 숨겨진 포인트를 재설정해야 합니다. 필드 이름이 다른 경우 <b>설정의 데이터 탭</b> 아래에서 필드 설정을 재설정해야 합니다.",
      ...
      "dataPicError4": "이렇게 하면 지원되지 않는 그림 URL 끝에 <i>#isImage</i>가 추가됩니다. 대부분의 서버는 URL 추가 요소를 지원합니다. 단, 이 작업을 수행한 후에는 포인트를 통해 이동하여 업데이트된 그림 URL이 작동하는지 확인해야 합니다. 각 그림을 불러오면 이제 맵 투어를 저장할 수 있습니다. <b>그림이 손상된 경우에는 맵 투어를 저장하지 않습니다.</b> 대신 빌더를 다시 불러오고 두 번째 작업을 수행합니다.",
      ...
      "dataPicError7": "맵 투어는 사진으로 제한되어 있으므로 비디오를 사용할 수 없습니다. 해당 제한을 제거하기로 선택한 경우 지도 맵 투어를 저장하기 전에 사진을 올바르게 불러오는지 확인합니다. 필요한 경우 나중에 제한을 복원할 수 있습니다.",
      ...
      "settingsLayoutSidePanel": "사이드 패널",
      ...
      "settingsExtentExplain": "아래의 대화형 맵을 통해 맵 투어 홈 범위를 설정합니다.",
      ...
      "dataTitle": "호스팅되는 맵 투어 레이어 생성",
      "dataExplain": "이미지 및 투어 포인트에 대해 새 피처 레이어가 생성됩니다. 맵 투어를 공유하지 않으면 투어 레이어는 공유되지 않습니다.",
    ...
    "viewFlickr": {
      "title": "Flickr 가져오기",
      "header": "플리커(Flickr) 사용자 이름을 입력하고 가져올 사진 앨범 또는 태그를 선택합니다.",
    ...
    "helpPopup": {
      ...
      "tab1": {
        "title": "소개",
        "moreInfo": "자세한 내용은",
        "infoSites": "<a href='https://storymaps.arcgis.com' target='_blank'>Esri 스토리맵 웹 사이트</a>의 <a href='https://storymaps.arcgis.com/en/app-list/map-tour/' target='_blank'>맵 투어 섹션</a>을 방문하세요.",
        "mtFAQ": "<a href='https://storymaps.arcgis.com/en/faq/#category4' target='_blank'>맵 투어 FAQ를 참고하세요.</a>",
        "myStories": "맵 투어를 관리하고 웹 사이트에서 <a href='https://storymaps.arcgis.com/en/my-stories/' target='_blank'>내 스토리</a>에 있는 오류를 확인할 수 있습니다.",
        ...
        "div42": "다른 사용자가 생성하는 맵 투어의 예시를 보려면 <a href='https://storymaps.arcgis.com/' target='_blank'>스토리맵 웹 사이트</a>에서 <a href='https://links.esri.com/storymaps/map_tour_gallery' target='_blank'>갤러리를 방문</a>하세요. Twitter에서 <a href='https://twitter.com/EsriStoryMaps' target='_blank'>@EsriStoryMaps</a>로 팔로우할 수도 있습니다.",
      ...
      "tab2": {
        "title": "데이터",
        "div1": "맵 투어를 생성할 때 고려해야 할 기본 요소는 그림을 저장할 위치를 선택하는 것입니다. 맵 투어에서는 사진 공유 서비스 및 웹 서버에 저장된 사진 또는 디스크에서 업로드한 사진을 사용할 수 있습니다.",
        "div1a": "지원되는 그림 형식 및 동영상에 대한 자세한 내용은 이 탭의 마지막 섹션을 참조하세요.",
        "div2": "대화형 빌더에서는 Map Tour에서 이미지를 처리할 수 있는 두 가지 옵션을 제공합니다.",
        "div3": "<ul><li>플리커 또는 Google+ 등의 이미지나 자신의 웹 서버에 저장된 이미지와 같이 <b>이미 온라인에 있는 사진</b>을 사용할 수 있습니다. 이러한 이미지는 해당 URL을 통해 맵 투어에서 참조됩니다.</li><li><b>컴퓨터에서 맵 투어로 직접 사진을 업로드</b>할 수도 있습니다. 이 업로드 옵션을 사용하려면 사진이 첨부 파일로 저장되는 호스팅된 피처 서비스가 자동으로 생성되므로 ArcGIS 구독이 있어야 하며 발행자 또는 관리자 권한이 있어야 합니다.</li></ul>",
        ...
        "div10": "웹 서버에 그림 저장",
        "div11": "플리커 또는 Google+의 이미지에 접근하거나 투어에 직접 업로드하는 대신, 자신의 웹 서버에서 그림을 호스팅하도록 선택하는 경우 수동으로 그림의 썸네일을 만들어야 합니다. 썸네일에 전체 해상도 그림을 사용하면 성능이 저하됩니다. URL을 통해 웹에서 직접 이미지에 접근할 경우 이 <a href='https://links.esri.com/storymaps/faq_maptour_image_size' target='_blank'>FAQ</a>를 참고하여 이미지 크기 조정에 대한 팁을 얻으세요.",
        "div12": "기존 GIS 데이터 사용",
        "div13": "응용프로그램에 필요한 필드 집합이 포함된 경우 모든 피처 서비스, CSV 파일 또는 포인트를 포함하는 쉐이프파일을 맵 투어 데이터 원본으로 사용할 수 있습니다. 이러한 서비스 또는 파일을 ArcGIS 웹 맵에 레이어로 추가한 다음, 맵 투어를 생성하여 맵을 공유해야 합니다.",
        ...
        "div174": "기존 GIS 데이터 사용에 대한 자세한 내용은 이 <a href='https://links.esri.com/storymaps/faq_map_tour_gis_data' target='_blank'>FAQ</a>를 참고하세요.",
        ...
        "div21": "플리커 또는 Google+에서 이미지에 접근하면 맵 투어에서 해당 이미지에 .jpg 파일로 자동 접근합니다. 맵 투어 빌더에 이미지를 업로드하는 경우 이미지는 .jpg 또는 .jpeg 형식이어야 합니다. 웹에서 URL을 통해 이미지를 직접 참조하는 경우 지원되는 그림 형식은 .jpg, .jpeg, .png, .gif, .bmp입니다. YouTube 비디오가 바로 지원됩니다.",
      ...
      "tab4": {
        ...
        "div1": "이미지",
        "div2": "하나의 맵 투어에서 다양한 크기, 도형 및 방향의 이미지를 사용할 수는 있지만 세로 방향보다는 가로 방향의 이미지를 사용하는 것을 권장합니다. 경사면 방향 비율이 4:3인 이미지가 가장 적합합니다. 캡션이 이미지와 중첩되는 세 개의 패널 및 통합 레이아웃에서 세로 방향 이미지를 사용하면 많은 이미지가 캡션에 의해 가려질 수 있습니다. 세 개의 패널 및 통합 레이아웃에서는 가능한 경우 크기와 도형이 모두 동일한 이미지를 사용하는 것을 권장합니다. 그러면 사용자가 투어를 진행할 때 크기가 다른 이미지 때문에 혼동되는 일이 발생하지 않습니다.",
        ...
        "div5": "보조 레이어",
        "div6": "보조 레이어를 추가하여 맵 투어에 컨텍스트를 제공할 수 있습니다. 이러한 레이어는 맵 투어 포인트 외에 연구 영역, 투어 포인트에 연결되는 도보 또는 운전 경로 등 맵에 표시할 기타 지리적 피처를 포함할 수 있습니다. 맵 투어 템플릿은 웹 맵에서 지정한 심볼을 사용하여 이러한 추가 지원 레이어를 표시하지만, 레이어에 정의된 팝업은 지원되지 않습니다.  투어에 지원 레이어를 추가하려면 작업을 저장한 후 맵 투어 빌더를 닫습니다. 그런 다음, 투어에 사용된 웹 맵을 엽니다. 이러한 지원 레이어는 ArcGIS 내 콘텐츠나 스토리맵 웹 사이트의 내 스토리 섹션에서 찾을 수 있습니다.",
        "div7": "간결하게 Tour 유지",
        ...
        "div10": "포함 모드",
        "div11": "iframe을 통해 다른 웹 사이트에 템플릿을 포함하려는 경우 URL 끝에 선택적 매개변수 \"&embed\"를 추가하면 헤더가 제거됩니다.",
        "div12": "너비가 좁은 iFrame은 맵 투어를 반응형 소형 디스플레이 뷰로 전환되게 하므로 사용하지 마세요. 사용 편의성을 최대화하려면 맵 투어를 임베드할 때 사용자가 투어를 전체 화면으로 실행할 수 있도록 임베드된 투어 옆에 링크를 제공하는 것을 권장합니다."
      },
      "tab5": {
        "title": "발행",

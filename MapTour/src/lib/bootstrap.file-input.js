/*
  ...
  Esri - 02/2013 - Copy node css classes into the new node to allow customization of the button
...
$('input[type=file]').each(function(i,elem){
  ...
  // Give the new button the original html classes
  var classes = $(this).attr('class')

  // Now we're going to replace that input field with a Bootstrap button.
  // The input will actually still be there, it will just be float above and transparent (done with the CSS).
  $(elem).replaceWith('<a class="file-input-wrapper btn ' + classes + '">'+buttonWord+input+'</a>');
})

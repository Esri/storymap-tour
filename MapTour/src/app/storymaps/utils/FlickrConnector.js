define([], function(){
	return function FlickrConnector(apiKey)
	{
		...
		{
			var connectDeferred = new dojo.Deferred();
		...
		{
			return getPictures('flickr.photosets.getPhotos', dojo.mixin({photoset_id: setId}, options));
		}
		
		this.getPicturesByTag = function(tags, options)
		{
			return getPictures('flickr.photos.search', dojo.mixin({tags: tags}, options));
		}
		...
		function getPictures(method, params)
		{
			...
			request(method, searchParams).done(function(result){
				...
				$.each(photos, function(i, photo) {
					response.push({
						name: photo.description ? (photo.description._content || '') : '',

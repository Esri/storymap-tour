define(["storymaps/ui/crossfader/CrossFader",
	...
	return function PicturePanel(selector, isInBuilderMode)
	{
		...
		function init(bgColor)
		{
			...
			dojo.subscribe("CROSSFADER_CHANGE", function(width, height){
		...
		function update(bgColor)
		{
			...
			if (MapTourHelper.isModernLayout()) {
				...
				$("#cfader").css("margin-top", 2.7 * VERTICAL_MARGIN);
			}
			else {
		...
		function updatePicture(picurl, name, caption, buttonStatus)
		{
			...
			if ( buttonStatus.left && MapTourHelper.isModernLayout() )
			...
			if( buttonStatus.right && MapTourHelper.isModernLayout()  ) 
		...
		{
			if( MapTourHelper.isModernLayout() )
				resizeModernLayout(panelAvailableWidth, panelAvailableHeight);
			else
				resizeRegularLayout(panelAvailableWidth, panelAvailableHeight);
		...
		function resizeModernLayout(panelAvailableWidth, panelAvailableHeight)
		{
			...
			var availableHeight = panelAvailableHeight - (4 * VERTICAL_MARGIN);
			...
			width = Math.min(parseInt(availableHeight * 4/3), availableWidth);
			...
			setTimeout(function(){
				$("#arrowPrev").css("top", (panel.height() / 2) - $(".member-image.current").height() / 2 + 14);
				$("#arrowNext").css("top", (panel.height() / 2) - $(".member-image.current").height() / 2 + 14);
		...
		function resizeCommon()
		{
			...
			if( $("#placardContainer").outerHeight() > imgDisplayedHeight / 1.5 ) {
				...
				if ( !isInBuilderMode && imgDisplayedHeight ) {
					...
					//$("#placardContainer .name").css("max-height", imgDisplayedHeight / 5);
				}
			}
			else {
				$("#placard").removeClass("one-line-layout");
				if( ! isInBuilderMode && imgDisplayedHeight )
					$("#placardContainer .description").css("max-height", "inherit"); // #placardContainer .name, 
			}
				
			$("#placard-bg").css("max-height", imgDisplayedHeight - 10 - (MapTourHelper.isModernLayout() ? 3 : 0));
			
			// Show builder button bar if a picture is loaded
			if ( isInBuilderMode && app.data.getCurrentIndex() != -1 ) {
				...
				if( ! MapTourHelper.isModernLayout() )
					$(".editPictureButtons").css("margin-left", dojo.query("#cfader .current").length ? dojo.query("#cfader .current").position()[0].x : 0);
				else 
					$(".editPictureButtons").css("margin-left", dojo.query("#cfader .current").length ? dojo.query("#cfader .current").position()[0].x - dojo.query("#cfader").position()[0].x : 0);
				...
				// Position button bar
				$(".editPictureButtons").css("top", parseFloat($("#cfader > img.current").first().css("top")) + 24 + (MapTourHelper.isModernLayout() ? 31 : 0));
			}
			else
			...
			if( pictureChanged ) {
				...
				dojo.publish("CORE_PICTURE_CHANGED");
			}
		}
		...
		function initBuilderFileReader()
		{
			...
			_changePictureInput.change(function()
			{
				_changePictureBtn.popover('show');
			});
			...
			_changeThumbnailBtn.change(function()
			{
		...
		{
			var file = _changePictureInput.get(0).files[0];
			if( ! confirmed || ! file ) {
				clearPictureInput();
		...
		{
			_changePictureInput.closest("form").get(0).reset();
		}
